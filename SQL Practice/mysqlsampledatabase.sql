...
select * from employees where (employeeNumber = 1102 AND firstName = "Mary") OR (employeeNumber = 1056 AND firstName = "Mary");
select * from employees where firstName NOT IN ("Mary");
select * from employees where employeeNumber BETWEEN 1000 AND 1145;

-- Gives you the number of rows in orders table:
select count(*) from orders;

-- This is how you comment a single line in SQL.
...
-- various quaries below are unrelated to this database

select * from products
where quantityInStock IN (49, 39, 72); -- unrelated

select * from customers where dateOfBirth BETWEEN "1990-01-01" AND "2000-01-01"; -- unrelated

select * from customes where address LIKE "%trail%" OR "%avenue%"; -- unrelated

select * from employees -- related
where firstName 
REGEXP "Elka|Ambur"
OR lastName 
REGEXP "EY$|ON$" 
...
select contactFirstName, contactLastName, 10 + 10 as points from customers order by postalCode DESC, state, 3, 1, 2 limit 20; --- related
select *, quantity * unit_price as total_price from order_items where order_id= 2 order by total_price DESC; -- unrelated to data
